...
public class NumberOperations {
    ...
    */
    ...
    public static void main(String[] args) {
        ...
        Runnable averageValue = () -> {
            System.out.println(numberList.stream().mapToInt(Integer::intValue).sum() / numberList.size());
        };
        Runnable sumEachSecond = () -> {
            System.out.println(IntStream.range(0, numberList.size())
                    .filter(n -> n % 2 == 0)
        ...
        executor.execute(sumNumbers);
        executor.execute(averageValue);
        executor.execute(sumEachSecond);
        executor.shutdown();
    }
