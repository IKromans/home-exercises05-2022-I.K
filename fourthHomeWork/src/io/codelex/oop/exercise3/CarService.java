...
public class CarService {
    ...
    public List<Car> manufacturetAtLeast(int count){
        return carsList.stream()
                .filter(car -> car.manufactureList.size() >= count )
                .toList();
    }
    ...
    public List<Car> specificManufacturer(String manufacturer){
        return carsList.stream()
                .filter(car -> car.manufactureList.get(0).getName().equals(manufacturer))
                .toList();
    }
}
